...
if not os.path.exists(hp.wav_path):
    raise FileNotFoundError(f"Dataset not found: {hp.wav_path}")
...
#         dataset = list(pool.imap_unordered(process_wav, wav_files))
...
if __name__ == "__main__":
    ...
    if len(wav_files) == 0:
        ...
        n_workers = 1  # Force single-threaded processing
        ...
        simple_table([
            ...
            ('Mu Law', hp.mu_law),
            ('Hop Length', hp.hop_length),
            ('CPU Usage', 'Single-threaded (No multiprocessing)')
        ])

        dataset = []
        
        # Process files one by one (no multiprocessing)
        for i, wav_file in enumerate(wav_files, 1):
            item_id, length = process_wav(wav_file)  # Directly call the function
            dataset.append((item_id, length))
